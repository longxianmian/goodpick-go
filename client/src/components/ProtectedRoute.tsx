...
export function ProtectedRoute({ children, requiredRole, showBottomNav = false }: ProtectedRouteProps) {
  ...
  if (authPhase === 'booting') {
    return (
      <div className={`flex items-center justify-center min-h-screen bg-background ${showBottomNav ? 'pb-14' : ''}`}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#38B03B] mx-auto"></div>
          <p className="mt-3 text-sm text-muted-foreground">{t('common.loading')}</p>
        </div>
        {showBottomNav && <UserBottomNav />}
      </div>
    );
  }
  ...
  if (!isUserAuthenticated || !user) {
    return (
      <div className={`flex items-center justify-center min-h-screen bg-gradient-to-b from-[#38B03B]/10 to-background p-4 ${showBottomNav ? 'pb-20' : ''}`}>
