...
export async function requestMicrophonePermission(): Promise<boolean> {
  ...
  // 直接使用标准 Web API - LIFF permission API 不支持 'microphone'
  // 经过调试发现 liff.permission.query('microphone') 会抛出 'Unexpected permission name' 错误
  console.log('[LiffClient] 使用 getUserMedia 请求麦克风权限');
  debugLog('calling getUserMedia directly');
  ...
  try {
    ...
    debugLog('getUserMedia error', { error: error?.message || String(error) });
    return false;
  }
}
